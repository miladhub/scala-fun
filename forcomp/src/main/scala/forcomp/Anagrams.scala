...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences isEmpty)
      ...
      val subs : List[Occurrences] = 
        children.map(c => subtract(occurrences, List(c)))
      
      occurrences :: subs.flatMap((sub : Occurrences) => combinations(sub)).toSet.toList
    }
  }
    ...
    def loop(occurrences: Occurrences): List[Sentence] = {
		  if (occurrences.isEmpty)
		    ...
		    for {
		      ...
		      sentence <- loop(subtract(occurrences, subset))
	      } yield word :: sentence
    }
  ...
  def sentenceAnagramsMemo(sentence: Sentence): List[Sentence] = {
    ...
    def loopMemo(occurrences: Occurrences): List[Sentence] = {
    ...
    def loop(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
		    ...
		    for {
		      ...
		      if (dictionaryByOccurrences.contains(subset))
		      word <- dictionaryByOccurrences(subset)
		      sentence <- loopMemo(subtract(occurrences, subset))
	      } yield word :: sentence
    }
    loopMemo(sentenceOccurrences(sentence))
  }
}
