...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] =
      if (isStanding)
        List(
          (Block(Pos(b1.x, b2.y + 1), Pos(b2.x, b2.y + 2)), Right),
          (Block(Pos(b1.x, b1.y - 2), Pos(b2.x, b2.y - 1)), Left),
          (Block(Pos(b1.x - 2, b1.y), Pos(b2.x - 1, b1.y)), Up),
          (Block(Pos(b1.x + 1, b1.y), Pos(b2.x + 2, b2.y)), Down)
        )
      else if (b1.x == b2.x)
        List(
          (Block(Pos(b1.x, b1.y + 2), Pos(b2.x, b2.y + 1)), Right),
          (Block(Pos(b1.x, b1.y - 1), Pos(b2.x, b2.y - 2)), Left),
          (Block(Pos(b1.x - 1, b1.y), Pos(b2.x - 1, b2.y)), Up),
          (Block(Pos(b1.x + 1, b1.y), Pos(b2.x + 1, b2.y)), Down)
        )
      else
        List(
      		(Block(Pos(b1.x, b1.y + 1), Pos(b2.x, b2.y + 1)), Right),
          (Block(Pos(b1.x, b1.y - 1), Pos(b2.x, b2.y - 1)), Left),
          (Block(Pos(b1.x - 1, b1.y), Pos(b2.x - 2, b1.y)), Up),
          (Block(Pos(b1.x + 2, b1.y), Pos(b2.x + 1, b2.y)), Down)
        )
